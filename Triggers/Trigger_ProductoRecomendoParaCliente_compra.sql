CREATE TRIGGER trg_ProductoRecomendadoParaCliente_compra
...
BEGIN
    ...
    SELECT i.clienteId, i.productoId, GETDATE(), 'Compro más de 3 veces este producto'
    FROM (
        SELECT clienteId, productoId
        FROM HistorialClienteProducto
        WHERE tipoAccion = 'Compra'
